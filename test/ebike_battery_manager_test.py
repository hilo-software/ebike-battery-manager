...
def test_update_battery_plug_list():
    with patch('kasa.SmartDevice') as mock:
        reset_device_thresholds()
        ...
        assert len(target.battery_plug_list) == 0
        target.update_battery_plug_list(mock_smart_device_plug, manufacturer_plug_names)
        ...
        target.update_battery_plug_list(mock_smart_device_strip, manufacturer_plug_names)
        assert len(target.battery_plug_list) == 3
        target.battery_plug_list = save_battery_plug_list

def test_update_battery_plug_list_config_name_not_battery():
    with patch('kasa.SmartDevice') as mock:
        reset_device_thresholds()
...
def verify_plug(plug: BatteryPlug, nominal: float, storage: float, full: float) -> None:
    plug_name = plug.name
    if plug_name in target.plug_storage_list:
        assert plug.battery_charge_mode == target.BatteryChargeMode.STORAGE
        assert plug.thresholds.storage_charge_battery_power_threshold == storage
        ...
        assert plug.get_active_charge_battery_power_threshold() == nominal
...
def test_storage_mode():
    ...
    assert len(target.plug_manufacturer_map) == 5
    assert len(target.plug_storage_list) == 1
    assert target.device_thresholds['Rad'].nominal_charge_battery_power_threshold == 90.0
    assert target.device_thresholds['Rad'].full_charge_battery_power_threshold == 5.0
    assert target.device_thresholds['Rad'].storage_charge_battery_power_threshold == 115.0
    assert target.device_thresholds['Lectric'].nominal_charge_battery_power_threshold == 80.0
    assert target.device_thresholds['Lectric'].full_charge_battery_power_threshold == 10.0
    assert target.device_thresholds['Lectric'].storage_charge_battery_power_threshold == 80.0
    for plug_name in list(target.plug_manufacturer_map.keys()):
        ...
        if 'rad' in plug_name:
            ...
            verify_plug(plug, 90.0, 115.0, 5.0)
        print(f'plug_name: {plug_name}')
        if 'rad' in plug_name:
            strip_plug: target.BatteryStripPlug = target.create_battery_strip_plug(plug_name, any, 0)
            verify_plug(strip_plug, 90.0, 115.0, 5.0)
        if 'lectric' in plug_name:
            plug: target.BatteryPlug = target.create_battery_plug(plug_name, any)
            verify_plug(plug, 80.0, 80.0, 10.0)
        if 'lectric' in plug_name:
            strip_plug: target.BatteryStripPlug = target.create_battery_strip_plug(plug_name, any, 0)
            verify_plug(strip_plug, 80.0, 80.0, 10.0)
...
def test_get_device_thresholds():
    ...
    assert rad_device_threshold.full_charge_battery_power_threshold == target.FULL_CHARGE_THRESHOLD_DEFAULT
    assert rad_device_threshold.nominal_charge_battery_power_threshold == target.NOMINAL_CHARGE_THRESHOLD_DEFAULT
    assert rad_device_threshold.storage_charge_battery_power_threshold == 115.0
...
def test_threshold_check():
    nominal_thresholds = DeviceThresholds(rad_thresholds.manufacturer_name,
                                          rad_thresholds.nominal_charge_battery_power_threshold,
                                          rad_thresholds.full_charge_battery_power_threshold,
                                          rad_thresholds.storage_charge_battery_power_threshold,
                                          rad_thresholds.storage_charge_battery_cycle_limit,
                                          20)
...
def test_mock_BatteryPlug():
    with patch('ebike_battery_manager.BatteryPlug') as mock:
...
def test_mock_BatteryStripPlug():
    with patch('ebike_battery_manager.BatteryStripPlug') as mock:
        instance = mock.return_value
        instance.get_power.return_value = 4.5
        result = instance.get_power()
        assert result == 4.5
